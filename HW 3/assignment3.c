...
void sendSum(unsigned char str_length, unsigned int* arr);
unsigned int* storeDigits(unsigned int num, unsigned char str_length);
unsigned char getDigit(unsigned int num);
...
void Main(void) {
	...
	unsigned int  firstOp       = 0;		// First operand of the calculator
	unsigned int  secondOp      = 0;		// Second operand of the calculator
	unsigned int  intSum        = 0;
	unsigned char length        = 0;
	unsigned char str_length    = 0;
	unsigned char firstOp_done 	= 0;
	unsigned int* arr;
	...
	while(1){
		...
		for (i=0;i<4;++i){ // loop over the 4 rows
			...
			if (col != 0x70){ // If any column is zero i.e. a key is pressed
				...
				if (gotkey == '=' || gotkey == '#'){
					sendChar('=');
					intSum = firstOp + secondOp;
					str_length = getDigit(intSum);
					arr = storeDigits(intSum, str_length);
					sendSum(str_length, arr);
					break;
				}
				
				if (!firstOp_done){ 		
					firstOp = firstOp * 10 + (gotkey - '0');
					sendChar(gotkey);
				}
				
				if (firstOp_done && (gotkey != '+' && gotkey != '*')){ 		
					secondOp = secondOp * 10 + (gotkey - '0');
					sendChar(gotkey);
				}
		...
		if (gotkey == '=' || gotkey == '#'){break;}  // Since a key was detected -> exit the while loop
